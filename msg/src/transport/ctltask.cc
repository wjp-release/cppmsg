...
void connect_task::handle_async_connect_result(){
    int result=msg::common::get_sockfd_err(this->newfd);
    if(result<0){  
        if (result == EINPROGRESS) return; 
        else on_failure(fatal); // on_failure should release this connector object
    }else{
        on_success(0); // on_success should release this connector object
    }
}
